...
INSTRUCTIONS="Based on the description below, please list the files and code " \
"for a production ready application. Provide the information as a compact, " \
"JSON-formatted dictionary where the keys are file paths (not directories, " \
"just files!) and the values are the code. Stick to that description, don't add " \
"anything else. Make the minimum amount of assumptions about the rest of the repo. \n"
